...
namespace caffe {
  ...
  void RotationNetSoftmaxLossLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
      CHECK_EQ(bottom.size(), 2) << "RotationNetSoftmaxLossLayer Layer takes two blobs as input.";
      CHECK_EQ(top.size(), 1) << "RotationNetSoftmaxLossLayer Layer takes a single blob as output.";
      top[0]->Reshape(bottom[0]->num(), bottom[0]->channels(), bottom[0]->height(), bottom[0]->width());
      softmax_bottom_vec_.clear();
      nClassLabel = this->layer_param_.rotationnet_softmax_loss_param().stride();
      using_upright = this->layer_param_.rotationnet_softmax_loss_param().using_upright();
      num = bottom[0]->num();
  ...
  template <typename Dtype>
  void RotationNetSoftmaxLossLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
      ...
      LOG(INFO) << "prob_data size: " << sizeof(prob_data)/sizeof(Dtype);
      ...
      LOG(INFO) << "prob_ shape: (num=" << prob_.num() << ", channel=" << prob_.channels() << ", height="<< prob_.height() << ", width="<< prob_.width() << ")";
      LOG(INFO) << "Bottom 0 shape: (num=" << bottom[0]->num() << ", channel=" << bottom[0]->channels() << ", height="<< bottom[0]->height() << ", width="<< bottom[0]->width() << ")";
      LOG(INFO) << "Bottom 0 count: " << bottom[0]->count();
      LOG(INFO) << "Bottom 1 shape: (num=" << bottom[1]->num() << ", channel=" << bottom[1]->channels() << ", height="<< bottom[1]->height() << ", width="<< bottom[1]->width() << ")";
      LOG(INFO) << "nRotation = " << nRotation << ", nClassLabel = " << nClassLabel;
      LOG(INFO) << "nSample = " << nSample;
      ...
      // loss calculation
      for (int i = 0; i < num; ++i) {
        loss += -log(max(prob_data[i * dim + labelR[i] ], Dtype(FLT_MIN)));
        int id = labelR[i] / nClassLabel;
        for (int j = 0; j < nRotation; ++j) 
          if( j != id )
            loss += -log(max(prob_data[i * dim + j * nClassLabel + nClassLabel - 1 ], Dtype(FLT_MIN)));
      }
      Dtype lossValue = loss / (num * nRotation);
      top[0]->mutable_cpu_data()[0] = lossValue;
      LOG(INFO) << "RotationNetLoss = " << lossValue; 
  }

  template <typename Dtype>
  void RotationNetSoftmaxLossLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  INSTANTIATE_CLASS(RotationNetSoftmaxLossLayer);
}