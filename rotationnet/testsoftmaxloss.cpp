...
void layerSetUp();
void generateGausian(float* data, int n);
...
int main(int argc, char** args) {
      ...
      float label[240] = {    24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24,
      ...
      vector<int> max_ang;
      float* prob_data = new float[count];
      float tmp_prob;

      //Generate data
      generateGausian(prob_data, count);
      ...
      for (int n = 0; n < nSample; ++n) {
            ...
            for (int i = 0; i < nRotation; ++i) {
                  ...
                  for (int j = 0; j < nRotation; ++j) {
                        ...
                        tmp_prob += log( max(prob_data[ ( n * nRotation + idx ) * dim + j * nClassLabel + label_ ], float(FLT_MIN)) );
                        tmp_prob -= log( max(prob_data[ ( n * nRotation + idx ) * dim + j * nClassLabel + nClassLabel - 1 ], float(FLT_MIN)) );
                  }
      ...
      // loss calculation
      for (int i = 0; i < num; ++i) {
            loss += -log(max(prob_data[i * dim + labelR[i] ], float(FLT_MIN)));
            int id = labelR[i] / nClassLabel;
            for (int j = 0; j < nRotation; ++j) 
                  if( j != id )
                        loss += -log(max(prob_data[i * dim + j * nClassLabel + nClassLabel - 1 ], float(FLT_MIN)));
      }
      float lossValue = loss / (num * nRotation);
      cout << "RotationNetLoss = " << lossValue << endl;
