...
namespace caffe {
	...
	template<typename Dtype> void RotationNetAccuracyLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {

		top[0]->Reshape(1, 2, 1, 1);
		//nClassLabel = this->layer_param_.my_accuracy_param().stride();
		nClassLabel = this->layer_param_.rotationnet_accuracy_param().stride();
		// using_upright = this->layer_param_.my_accuracy_param().using_upright();
		using_upright = this->layer_param_.rotationnet_accuracy_param().using_upright();
	...
	template<typename Dtype> void RotationNetAccuracyLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
		Dtype accuracy = 0;
  		Dtype logprob = 0;
  		...
  		vector<Dtype> scores( nClassLabel - 1 );
  		...
  		for (n = 0; n < nSample; ++n) {
    		...
    		if(using_upright) {
    				...
    				for(c = 0; c < nClassLabel-1; ++c) {
    					...
    					for (j = 0; j < nRotation; ++j) {
	               			...
	               			tmp_prob += log( bottom_data[ ( n * nRotation + idx ) * dim + j * nClassLabel + c ] ) - 
	               						log( max(bottom_data[ ( n * nRotation + idx ) * dim + j * nClassLabel + nClassLabel - 1 ], Dtype(FLT_MIN)) );
	           			}
    			...
    			//Case 2: without using upright
    			for (i = 0; i < ang.size(); ++i) {
	       			for (c = 0; c < nClassLabel - 1; ++c) {
	           			tmp_prob = 0;
	           			for (j = 0; j < nRotation; ++j) {
	              			tmp_prob += log( bottom_data[ ( n * nRotation + ang[ i ][ j ] ) * dim + j * nClassLabel + c ] ) - 
	              						log( max(bottom_data[ ( n * nRotation + ang[ i ][ j ] ) * dim + j * nClassLabel + nClassLabel - 1 ], Dtype(FLT_MIN)) );
	           			}
          	 			if( tmp_prob > max_prob ) {
          	    			max_ang = i;
          	    			max_prob = tmp_prob;
	...
	// #ifdef CPU_ONLY
	// 	STUB_GPU(RotationNetAccuracyLayer);
	// #endif
	
	INSTANTIATE_CLASS(RotationNetAccuracyLayer);
	// REGISTER_LAYER_CLASS(RotationNetAccuracy);
}